...
def create_localmetadata(db: Session, GameID: str, localmetadata: schemas.LocalMetadataCreate):
    from main import LocalMetadataModel
    check_row = db.query(LocalMetadataModel).filter(LocalMetadataModel.GameID == GameID).first()
    if check_row:
        db.delete(check_row)
